...
$.getJSON('JsonData/repairT_Data.php',{data: id.data},function (data) { 
        ...
        $('div#Rr_content').append($("<div class='row'><div class='col-md-3 col-xs-12'><div class='form-group'><label for='datepicker1' class='control-label'>วันที่เริ่มซ่อม </label><input type='text' name='datepicker1' id='datepicker1' class='form-control' readonly required></div></div>"
                                    ...
                                    ,$("<input type='hidden' id='repairT_id' name='repairT_id'>")
    ...
    $('div.modal-body#modelacc-body').append("<form name='frmacc' id='frmacc'></form>");
            $('#frmacc').empty().append($("<div class='form-group' id='accpart_sel'></div>")
                    ...
                    $("#accModal").find('.modal-title').text('เปลี่ยนอุปกรณ์ใบแจ้งซ่อม : ลำดับที่ ' + data.repairT_id);
                    $("#accModal").find('.modal-body input#repairT_id').val(data.repairT_id);
                    $("#accModal").find('.modal-body input#submethod').val('add_accT');   
    ...
    $('div.modal-body#modelsendre-body').append("<form name='frmsendre' id='frmsendre'></form>");
            $('#frmsendre').empty().append($("<div class='form-group'><label for='datepicker3' class='control-label'>วันที่ส่งซ่อม</label><input type='text' name='datepicker3' id='datepicker3' class='form-control' readonly required></div>")
                    ...
                    $("#sendreModal").find('.modal-title').text('ส่งซ่อมภายนอก ใบแจ้งซ่อม : ลำดับที่ ' + data.repairT_id);
                    $("#sendreModal").find('.modal-body input#repairT_id').val(data.repairT_id);
                    $("#sendreModal").find('.modal-body input#submethod2').val('add_sendRepairT');   
                                               ...
                                               $.getJSON('JsonData/DT_TRPT.php',function (data) {
                                                   if(data.send_repair !=0){
                                                    ...
                                                    if(data.list_repair !=0){
                                                        ...
                                                        $("#listResult2 small").remove()
                                                    }
                                                        });  
                                                $("#index_content").empty().load('content/list_repairT_result.html');
                                                 return false;
					   }
					 });
					   }
         ...
         $("#send_repair0").click(function(){
            ...
            $("#rece_pd_sel").show("fast"); 
                    ...
                    $.getJSON('JsonData/emp_Data.php', function (GD) {
                                ...
                                $("input#method").val('record_repairT');
                                $("input#repairT_id").val(data.repairT_id);
