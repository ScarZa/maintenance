function RefreshRepairT() {
   $.getJSON('JsonData/count_repT.php', function (data) {
      $("#myTSpan").empty().append($("<a href='#' class='dropdown-toggle' data-toggle='dropdown'>"
         + "<i style='color: purple;' class='fa fa-bell'></i>"
         + "<span class='label label-info'>" + data.alertRepair + "</span></a>")
         , $("<ul class='dropdown-menu'><li class='header' style='color: red;'><b>คุณมี " + data.alertRepair + " รายการแจ้งซ่อม(ช่าง)</b></li>"
            ...
            + "<li class='footer'><a href='#' id='total_repT'>ดูทั้งหมด</a></li></ul>"));
      $("#total_repT").attr("onclick", "loadPage('#index_content','content/list_repairT_order.html')");
      $("");
      $.getJSON('JsonData/detail_repT.php', function (data) { console.log(data)
         for (var key in data) {
            ...
            $("#menu_Talert").append($("<li><a href='#' id='recM' data-toggle='modal' data-target='#receiveTModal' data-whatever='" + data[key].repairT_id + "'>"
